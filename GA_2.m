function [xGA, fval, fvalHistory] = GA_2(fn)
    ...
    if fn == "f1"
        ...
        for i = 1:agent_num %3x10x100?øΩ?øΩP?øΩ?øΩ?øΩ?ê¨?øΩ?øΩ?øΩ?øΩ
            P(i).g = de2bi(randi([0,2^32-1],1,3),n);%genotype?øΩÕÇQ?øΩi?øΩ?øΩ
            P(i).f = 0;
        end
    ...
    elseif fn == "f2"
        ...
        for i = 1:agent_num %3x12x100?øΩ?øΩP?øΩ?øΩ?øΩ?ê¨?øΩ?øΩ?øΩ?øΩ
            P(i).g = de2bi(randi([0,2^32-1],1,2),n);%genotype?øΩÕÇQ?øΩi?øΩ?øΩ
            P(i).f = 0;
        end
        evaluationFunction = str2func(fn);
    end
    ...
    for i = 1:ITERATION
        ...
        for j = 1:agent_num
            %tic;
            if fn == "f1"
                Phenotype = (bi2de(P(j).g)/(2^22)-511)/100;
                P(j).f = evaluationFunction(Phenotype(1),Phenotype(2),Phenotype(3)) ;
            end
            
            if fn =="f2"
                Phenotype = (bi2de(P(j).g)/(2^20)-2047)/100;
        ...
        for j = n_parent+1:agent_num
            ...
            %
                 P_next(j).g = crossover_2(P,parents,fn);
