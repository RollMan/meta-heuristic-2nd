function varargout = differentevolution(varargin)
...
while ...
    (iterationNr < DEParams.maxiter) && (c < 100 ) %&&...
    %(isempty(VTR) || isnan(VTR) || (bestval * minMaxSign > VTR * minMaxSign))
   if iterationNr > 3 && bestvalhist(end)==bestvalhist(end-1)
       c = c + 1;
   else
       c = 0;
   end
...
if nargout > 0
  ...
  varargout = {bestmem', bestval, bestFctParams, iterationNr, resultFileName,bestvalhist};
end
